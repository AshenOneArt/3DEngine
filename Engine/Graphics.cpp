...
void Graphics::DrawFlatTriangleTex(const TexVertex& v0, const TexVertex& v1, const TexVertex& v2, const Surface& tex, const TexVertex& dv0, const TexVertex& dv1, TexVertex& itEdge1)
{
	...
	for (int y = yStart; y < yEnd; y++, itEdge0 += dv0, itEdge1 += dv1)
	{
		...
		const Vec2 dtcLine = (itEdge1.texCoord - itEdge0.texCoord) / (itEdge1.pos.x - itEdge0.pos.x);

		// create scanline tex coord interpolant and prestep
		Vec2 itcLine = itEdge0.texCoord + dtcLine * (float(xStart) + 0.5f - itEdge0.pos.x);
...
void Graphics::DrawDownTriangle(Vec2 t0, Vec2 t1, Vec2 t2, Color c)
{
	...
	float slopeY1 = (t2.x - t1.x) / (t2.y - t0.y);
